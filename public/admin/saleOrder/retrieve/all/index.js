window.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchFilteredSaleOrders(queryParams = "") {
        fetch(`/saleOrders/search?${queryParams}`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
            ...
            .then(function (body) {
                ...
                tbody.innerHTML = "";
                saleOrders.forEach(function (saleOrder) {
                    if (saleOrder.saleOrderItem && Array.isArray(saleOrder.saleOrderItem)) {
                        ...
                        console.error("saleOrderItem is undefined or not an array:", saleOrder);
                    }
                });
            })
            .catch(function (error) {
                console.error(error);
                alert("Error fetching filtered sale orders: " + error.message);
            });
    }

    const newButton = document.querySelector("button[type='submit']");
    if (newButton) {
        newButton.addEventListener("click", handleFilteredFormSubmission);
    }

    function handleFilteredFormSubmission(event) {
        event.preventDefault();

        const formElements = Array.from(document.querySelector("form.search").elements);
        const formValues = formElements.reduce(function (values, element) {
            if (element.type !== "submit") {
                values[element.name] = element.value;
            }
        ...
        const status = Array.from(document.querySelector("form.search").elements.status.options)
            .filter(function (option) {
                return option.selected;
            })
            .map(function (option) {
        ...
        console.log("Filtered Query Params:", queryParams);

        fetchFilteredSaleOrders(queryParams);
    }
});